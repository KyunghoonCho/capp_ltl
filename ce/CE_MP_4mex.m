...
function [path_final,input_final,z_final,cost_final,flag] = CE_MP_4mex(sp,gp,R,param,map,sinit)
    ...
    while(n_traj < traj_num)
        ...
        switch R.dynamic
            case 'pointmass'
                ...
                [path_gen,ctrl_gen] = ce_generate_point_path(sp(1:2),gp(1:2),z,ce_param.m,R.dist_interval,R);
                Z(n_traj,:) = z;
            case {'unicycle_part','unicycle'}
                ...
                % [pseg,useg,is_outside] = ...
                %     ce_generate_trajectory(sp,z,ce_param.dim,ce_param.m,R.x_range,R.u_linear,R.dt);
        ...
        is_unsafe = checkFeasibility(path_gen,param.interval_collision,R,map);
        if(is_unsafe)
            n_traj = n_traj - 1;
            continue;
        end
